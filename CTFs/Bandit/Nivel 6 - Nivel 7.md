La solucion es algo parecido al del bandit6 los pasos son los siguientes: 

PASO 1: 
Analizamos donde aparecimos despues de habernos conectado al bandit6 y vemos que no tenemos nada si hacemos el comando ls ni con el comando ls -l solo cuando hacemos el ls -la es que vemos que aparecen un par de archivos ocultos entonces lo mejor que podemos hacer es empezar a buscar por las propiedades que nos dice la pagina y lo hariamos de la siguiente manera: 
...
-size 33c -> Nuestro tercer filtro. El archivo debe pesar  exactamente 33 bytes (la c al final espara que linux entienda que nos referimos a bytes)
...
Al ejecutar este comando el resultado sera una ruta en la cual esta el archivo que encaja con todas las propiedades y se acopla a todos los filtro que pusimos en el 'find' a esa ruta que nos arroja toca hacerlel un cat en mi caso quedaria de la siguiente manera: 
