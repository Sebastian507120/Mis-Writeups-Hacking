...
- **(-u)** `(Solo Únicos):` Es la opción clave
  ...
  - Con `-u`, le decimos: Elimina todo lo que se repita  y muéstrame solo lo que aparezca una única vez.

  Entonces luego de ejecutar el comando `sort data.txt | uniq -u` encontraríamos la contraseña para el **bandit9** que en mi caso es : `4CKMh1JI91bUIZZPXDqGanal4xvAg0JM`.