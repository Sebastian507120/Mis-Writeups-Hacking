El Problema: El texto del archivo **data.txt** está cifrado  con **ROT13**. Esto significa que cada letra del abecedario fue "rotada" 13 posiciones hacia adelante  (la A se volvió N, la B se volvió O, etc.) Necesito "destraducirlo" para leerlo.
...
- `(n-z):` Empieza desde la mitad (letra 13)hata el final.
 - `(a-m):` Luego da la vuelta y completa el principio.
 
 Y repito lo mismo `N-ZA-M` para que funcione también  con las Mayúsculas.

Luego de ejecutar el comando `(cat data.txt | tr 'a-zA-Z' 'n-za-mN-ZA-M')` nos estaria mostrando la contraseña del **bandit 12 que en mi caso es la siguiente: 7x16WNeHIi5YkIhWsfFIqoognUTyj9Q4