Este certificado acredita la finalización exitosa del curso de **Introducción a Linux** impartido por Marcelo Vázquez (S4vitar). Durante esta formación, adquirí las bases fundamentales para operar y maniobrar de forma fluida en sistemas operativos Linux. una competencia indispensable para el pentesting y la ciberseguridad.

![](../Capturas/Introducción%20al%20hacking%20(Hack4u)-20260629052158.png) 

## Detalles de la credencial: 
- **Emisor:** Hack4u Academy
- **Instructor:**  Marcelo Vázquez (S4vitar)
- **Fecha de expedición:** 29 de marzo de 2026
- **ID  de Credencial:** `4935-4520-5980-8849`

## Conocimientos Adquiridos
- Manejo avanzado de la interfaz de línea de comandos (CLI).
- Gestión de usuarios, grupos y sistema de permisos en entornos Unix/Linux.
- Navegación ágil, manipulación de archivos y compresión de la jerarquía del sistema.
- Fundamentos de administración de sistemas orientados a la auditoría de seguridad.